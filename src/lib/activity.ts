...
/**
 * Collects the latest track and pull request, sorted by recency.
 * Shared by the /api/activity route and the index page build step so both
...
export async function getActivity(): Promise<ActivityResponse> {
  ...
  const items = [latestPullRequest, latestTrack]
...
/**
 * Builds a fallback payload for prerendered pages. Never throws: if env
 * vars or upstream APIs are missing, the page renders with nulls and the
 * client fetch takes over after hydration.
 */
export async function getActivityFallback(): Promise<ActivityResponse> {
  try {
    return await getActivity();
  } catch {
    return { latest: null, latestTrack: null, latestPullRequest: null, items: [] };
  }
}
