  ...
  return (
    <RadixTooltip.Provider delayDuration={0}>
      <div className="flex flex-wrap gap-2 md:items-center md:flex-row">
        ...
        >
          <small>{AVAILABLE ? "Book me" : "Available Summer 2025"}</small>
        </Link>
        <Link
          delay={0.1}
          title="Github"
      ...
      <div>
        <FadeIn delay={delay ? 0.4 + delay : 0.4}>
          <a
            ...
            className={cn(
              "inline-flex justify-center items-center w-auto px-3 gap-1.5 h-8 text-secondary bg-gray-1 rounded-full font-medium duration-200 ease-out hover:scale-110 hover:text-primary hover:bg-gray-2 md:px-px md:w-8",
              className,
            )}
          >
            {children}
